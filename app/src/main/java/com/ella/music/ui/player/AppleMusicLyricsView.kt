...
internal fun nextSmoothLyricPositionMs(
    ...
    return when {
        ...
        // Was `delta > 80L`, so this branch only ever nudged the display *forward* to catch up
        // to a sample that had pulled ahead. Backward drift beyond backwardToleranceMs (frame
        // clock quietly running fast relative to the real audio clock — plausible over a whole
        // song from nothing more than measurement jitter in frameDeltaMs) had no correction at
        // all here: every frame kept returning `predicted` regardless of how far past tolerance
        // the gap grew, so the highlight would silently race ahead of the vocal for the rest of
        // the song until the drift finally crossed seekThresholdMs and hard-snapped backward —
        // exactly the "abrupt transition" this smoothing exists to avoid. abs(delta) makes the
        // same gentle quarter-correction apply symmetrically in both directions.
        abs(delta) > 80L -> predicted + (delta / 4L)
        else -> predicted
    }
}
