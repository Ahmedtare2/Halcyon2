...
internal fun CoverPlayerPage(
    ...
    BoxWithConstraints(modifier = modifier) {
        ...
        val onLiveCoverFrame: (Bitmap) -> Unit = { frame ->
            reactiveBackgroundScope.launch(Dispatchers.Default) {
                liveReactivePalette.value = PlayerPalette.from(frame, pagePalette.isLight)
            }
        }
        if (drawBackground && !useWidePlayer) {
            val sampledReactivePalette = liveReactivePalette.value
            if (wantsReactiveBackground && sampledReactivePalette != null) {
                DynamicCoverReactiveBackground(
                    livePalette = sampledReactivePalette,
                    modifier = Modifier.fillMaxSize()
                )
            } else {
                // Also the fallback for the first ~700ms of a dynamic-cover song, before the
                // first live frame has been sampled — avoids a flash of an empty/black backdrop.
        ...
        fun StyledPlayerArtwork(
            ...
            // See player_view_texture.xml: only the full-bleed immersive call sites need this.
            useTextureViewForDynamicSource: Boolean = false,
                ...
                when {
                    videoPlaybackActive && musicVideoVisible && musicVideoSource != null -> {
                        DynamicCoverVideo(
                            ...
                            // Was hardcoded to RESIZE_MODE_FIT regardless of what the caller
                            // asked for, so a music video always letterboxed even inside a
                            // Crop-requesting container (e.g. the Apple Music immersive cover)
                            // while the dynamic-cover (motion loop) branch right below correctly
                            // used dynamicResizeMode. Both sources should behave the same way.
                            resizeMode = dynamicResizeMode,
                            useTextureView = useTextureViewForDynamicSource,
                            onLivePaletteFrame = onLivePaletteFrame
                        )
                    }
        ...
        fun AppleMusicCoverPage(
            ...
            // Immersive Mode used to also require no dynamic cover/music video: the immersive
            // branch's bottom-fade mask (graphicsLayer Offscreen + a DstIn gradient) doesn't
            // work over a SurfaceView, so a motion cover or music video would visibly ignore the
            // fade. StyledPlayerArtwork now requests a TextureView for exactly this branch
            // instead, so the exclusion is no longer needed and was only ever hiding the
            // immersive treatment from the content most associated with "Immersive Mode."
                ...
                if (isAppleMusicImmersive) {
                    Box(
                        ...
                        StyledPlayerArtwork(
                            ...
                            useTextureViewForDynamicSource = true,
                            onLivePaletteFrame = onLiveCoverFrame,
        ...
        if (useWidePlayer && selectedPlayerPageStyle ==
            ...
            Column(
                ...
                if (immersiveAlbumCover) {
                    ...
                    val hasDynamicCoverSource = (videoPlaybackActive && musicVideoVisible && displayedMusicVideo != null) ||
                        (videoPlaybackActive && !musicVideoVisible && displayedDynamicCover != null)
                    Box(
                        modifier = Modifier
                            .fillMaxWidth()
