...
- 新增土耳其语与阿拉伯语支持，扩展至 10 种界面语言，完整适配 RTL 镜像排版布局。
- 全局默认字体与歌词西文字体内置 Inter Bold，中日韩补缺内置 MiSans Bold。
- 艺术家页面支持多封面轮播检视；长按封面进入画廊式全屏大图预览，支持左右滑动翻页检视、保存/分享底栏与 3 列缩略图画廊抽屉选择。
- 艺术家封面支持 MP4/MKV/WebM/MOV 本地视频作为动态背景，图频并存优先播放视频；长按预览页始终以视频作为第 1 项（`1/xxx`），首帧预览图支持点击播放/暂停。
- 深度适配小米 HyperOS 超级小岛 / 焦点通知歌词，支持动态胶囊与展开态歌词实时流转，提供独立设置开关。
- MV 沉浸歌词模式支持进入时默认隐藏系统状态栏与导航栏。
- 修复部分设备与系统版本下桌面歌词/通知权限状态刷新异常与特定音频编码标签写入问题。
- 修复逐字歌词高亮动画在部分场景下异常瞬间高亮整个单词的问题，恢复平滑沿着字符逐字扫描推进。
- 修复迷你播放条歌词与标题未跟随全局字体设置的问题。
- 修复 Last.fm 艺术家简介拉取触发 Cloudflare 盾时无法弹窗完成人机验证的问题，新增内置交互验证 Sheet。
- 歌词源全面支持 Lyrico Plugin API 5 规范插件，支持多源并发匹配与参数持久化。
...
- Bottom navigation adds "Search bar dock integration": search can merge into the custom dock entry; the collapsed mini player expands fully across the bar and displays lyrics.
- Added Turkish and Arabic translations, expanding to 10 supported interface languages with full RTL mirrored layout support.
- Built-in Inter Bold as default app and Western lyric font, with MiSans Bold fallback for CJK characters.
- Artist page supports multi-cover carousel; long-press opens a full-screen gallery preview with swipe navigation, bottom save/share pill bar, and a 3-column thumbnail drawer.
- Artist header supports local MP4/MKV/WebM/MOV videos as dynamic backgrounds, prioritizing video playback when both images and videos exist; full-screen preview always places the video first (`1/xxx`) with click-to-play/pause.
- Deep integration with Xiaomi HyperOS Super Island / Focus Notification lyrics, streaming real-time lyrics across dynamic capsule and expanded views with independent toggles.
- MV immersive lyrics mode defaults to hiding system status and navigation bars on entry.
- Fixed floating desktop lyric / notification permission refresh quirks on certain devices and tag writing on specific audio encodings.
- Restored smooth letter-by-letter karaoke sweep animation, fixing regression where whole words would highlight immediately.
- Fixed mini player lyric and title typography to properly inherit the global app font setting.
- Resolved Last.fm artist bio fetching Cloudflare challenge verification by integrating a dedicated interactive challenge Sheet.
- Upgraded online lyric matching engine to support Lyrico Plugin API 5 specification with concurrent multi-plugin searching and parameter configuration.

Version
- Version name: `1.2.9-beta1`
- Version code: `37`
